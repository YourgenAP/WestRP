$(function () {
    ...
    //     $.post('http://id_user/exit', JSON.stringify({}));
    //     return
    // })
    //when the user clicks on the confirm button, it will run
    $("#confirm").click(function () {
        let inputValue = $("#input").val()
        if (inputValue.length >= 100) {
            $.post("http://id_user/error", JSON.stringify({
        ...
        $.post('http://id_user/main', JSON.stringify({
            text: inputValue,
        }));
        return;
    })
